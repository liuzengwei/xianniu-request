...
class Request extends Vue {
  ...
  addPendingRequest(config) {
    ...
    if (this.pendingRequests.has(requestKey)) {
      ...
      if (cancelToken) {
        cancelToken.cancel('用户发起了新的相同请求')
      }
      this.pendingRequests.delete(requestKey)
    }

    // 如果配置中已经有 cancelToken，则使用现有的，否则创建新的
    let cancelToken
    if (config.cancelToken) {
      // 使用现有的 cancelToken
      this.pendingRequests.set(requestKey, { cancelToken: null, timestamp: Date.now(), hasExternalToken: true })
    } else {
      // 创建新的 cancelToken
      cancelToken = axios.CancelToken.source()
      config.cancelToken = cancelToken.token
      this.pendingRequests.set(requestKey, { cancelToken, timestamp: Date.now() })
    }
  ...
  applyRequestConfig(config, customConfig) {
    if (customConfig.method === 'formData') {
  ...
  async handleBlobResponse(blob, response) {
    ...
    const autoDownload = response.config.autoDownload !== false || response.autoDownload !== false
