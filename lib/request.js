    ...
    instance() {
        ...
        instance.interceptors.request.use(
            (config) => {
                ...
                const _fingerPrint = localStorage.getItem(this.fingerPrintKey) || ''
                config.headers[this.fingerPrintKey] = _fingerPrint
                if (_token) {
                    let token = ''
                    try {
        ...
        instance.interceptors.response.use(
            async (response) => {
                ...
                if (response.config.responseType === 'blob') {
                    try {
                        const _res = await this.blobToJson(res)
                        if (_res.code && _res.code !== 200) {
                             Message({
                                message: _res.msg || '未知错误',
                                type: 'error',
                                duration: 5 * 1000
                            })
                            return false
                ...
                // 处理响应
                if (!this.statusCode.SUCCESS.includes(res.code)) { // 如果请求不成功

                    const msg = res.msg || res[this.msgKey]
                    Message({
                        message: msg || '未知错误',
                        type: 'error',
                        duration: 5 * 1000
